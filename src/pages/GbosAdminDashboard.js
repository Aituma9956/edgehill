...
const GbosAdminDashboard = () => {
  ...
  const fetchAppraisals = async () => {
    try {
      ...
      setAppraisalsError('');
      const data = await appraisalAPI.getAllAppraisals(0, 100, appraisalStatusFilter, appraisalSearch);
      setAppraisals(data);
    } catch (error) {
  ...
  const renderAppraisalManagement = () => (
    <div className="main-content">
      ...
      <div className="dashboard-card">
          ...
          <table className="dashboard-table">
            ...
            <tbody>
              {appraisals.map(appraisal => (
                <tr key={appraisal.id}>
                  <td>{appraisal.student_number}</td>
                  <td>{appraisal.academic_year}</td>
                  <td>{appraisal.appraisal_period}</td>
                  <td>{appraisal.due_date}</td>
                  <td>
                    <span className="status-badge">{appraisal.status.replace(/_/g, ' ')}</span>
                  </td>
                  <td>
        ...
        {!appraisalsLoading && appraisals.length === 0 && (
